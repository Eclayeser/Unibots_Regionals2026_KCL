...
import ServoControllerGT2 as SC
import MotorsControllerGT2 as MC
from ball_detector_runtime_GT2 import (
    detect_balls,
    detect_obstacles,
...
# ══════════════════════════════════════════════════════════════════════════════
# PROCESS  –  AprilTag Detector // TODO (MANISH IMPROVED LOGIC)
# ══════════════════════════════════════════════════════════════════════════════
...
def motors_handler_thread(shared, worker_pause_event, stop_event):
    ...
    while not stop_event.is_set():
        ...
        if not shared["startingPosReached"]:
            # ── Initial startup: drive forward to the starting position ───────
    ...
    while not stop_event.is_set():
        ...
        if not shared["clawAdjusted"]:
            ...
            if shared["engageHandle"]:
                ...
                SC.mg90s_turn_by_180_down() #OR SC.mg90s_turn_by_180_up() to complete the full rotation and reset the handle for the next unload cycle
...
# ══════════════════════════════════════════════════════════════════════════════
# THREAD  –  Button Handler  (GPIO 4, via gpiozero) // FINISHED IMPLEMENTING THIS ONE
# ══════════════════════════════════════════════════════════════════════════════
...
def main():
    ...
    worker_pause_event.clear()   # start in paused state until button press
    timer_pause_event.clear()   # start in paused state until button press
