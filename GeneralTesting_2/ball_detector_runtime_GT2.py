...
try:
    from ball_detection_krishiv_GT2 import CAMERA_MATRIX, DIST_COEFFS
    from config_GT2 import (
        ATTRACTION_WEIGHT,
        BALL_DIAMETER_CM,
        BALL_TYPE_PRIORITY,
        DEBUG_STREAM_PORT,
...
def detect_balls(frame: np.ndarray, ping_pong_profile: str = DEFAULT_PING_PONG_PROFILE) -> dict:
    ...
    steel, steel_mask, steel_hough_circles, steel_kalman_point = _extract_steel(
        hsv, gray, calibration, accepted_ball_mask
    )
    detections = _prioritize_balls(steel + ping_pong)

    # Populate Kalman trajectory and ROI from tracker state
    steel_trajectory: list[tuple[int, int]] = (
        _STEEL_KALMAN.trajectory
        if _STEEL_KALMAN.is_initialized and _STEEL_KALMAN.moving and _STEEL_KALMAN.frames_tracked >= 3
        else []
    )
    steel_kalman_roi = _STEEL_KALMAN.get_roi(hsv.shape) if _STEEL_KALMAN.is_initialized else None
    ...
    result = {
        ...
        "steel_kalman_roi": steel_kalman_roi,
        "steel_kalman_point": steel_kalman_point,
        "steel_hough_circles": steel_hough_circles,
        "steel_trajectory": steel_trajectory,
    }

    if STREAM_DEBUG_FEED:
        start_debug_mjpeg_server()  # idempotent; no-op after first call
        push_debug_frame(annotate_debug_frame(undistorted.copy(), result))
